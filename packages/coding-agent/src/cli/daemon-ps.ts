...
import { readActiveOrphanProcesses, reapOrphanProcess } from "../core/orphan-process-journal.js";
	...
	for (const worker of findTrackedWorkers(supervisorSocketPath)) {
		...
		if (descriptor.orphanProcessJournalPath) {
			...
			for (const orphan of orphans) {
				// Legacy pid-only records cannot prove identity and never authorize a signal.
				...
				if (process.platform === "win32") {
					...
					await assertAdmission();
					if (reapOrphanProcess(orphan) && isProcessAlive(orphan.pid)) {
